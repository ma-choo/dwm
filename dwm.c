...
enum { SchemeNorm, SchemeSel, SchemeNormVacant, SchemeSelVacant }; /* color schemes --machoo */
...
static void showscratchpad(const Arg *arg);
...
static unsigned int scratchpad = 1 << LENGTH(tags);
struct Pertag {
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (!(occ & 1 << i || m->tagset[m->seltags] & 1 << i))
			drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSelVacant : SchemeNormVacant]);
		else	
	...
	tlpad = MAX((m->ww - ((int)TEXTW(stext) - lrpad)) / 2 - x, lrpad / 2);

	if ((w = m->ww - tw - x) > bh)
		drw_text(drw, x, 0, w, bh, tlpad, stime, 0);
...
monocle(Monitor *m)
{
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
		resize(c, m->wx - c->bw, m->wy - c->bw, m->ww, m->wh, 0); // borderless monocle --machoo
}
...
updatetime(void)
{
	...
	strftime(stime, 32, "%a %b %-d %-I:%M %p", tm_local);
}
...
setscratchpad(const Arg *arg)
{
	...
	c->mon->tagset[c->mon->seltags] |= c->tags = scratchpad;
	c->isfloating = True;
	resize(c, x, y, w, h, 0);
	arrange(m);
}
...
showscratchpad(const Arg *arg)
{
   ...
   for (c = selmon->clients; c && !(found = c->tags & scratchpad); c = c->next);
   if (found) {
       unsigned int newtagset = selmon->tagset[selmon->seltags] ^ scratchpad;
       if (newtagset) {
           selmon->tagset[selmon->seltags] = newtagset;
           focus(NULL);
           arrange(selmon);
